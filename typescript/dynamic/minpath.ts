function dpTable(matrix:Array<number>[],row:number,col:number):number{
    //暴力递归转动态规划要点
    /**
     * 1.确认所需要的位置
     * 2.确认所有basic case的值
...
//暴力递归
function processing(matrix:Array<number>[],row:number,col:number,i:number,j:number):number{
    if(i==row&&j==col){
        //basic case 
        return matrix[i][j];
    }
...
function minPath(matrix:Array<number>[]):number{
