...
export class BinaryTree{
    ...
    private initTree():BinaryNode{
        let stack:Array<BinaryNode>=[];
        let flag=0;
        let curNode:BinaryNode=null;
    ...
    private getItem(index:number):{ val : string | number,index : number}{
        let char='';
        let reg=new RegExp('[\(\)\,\#]');
        ...
        char=(isNaN(+char)?char:+char) as string;
        return {val : char,index : index};
    }

    preOrderTree(node:BinaryNode):void{
        let root=node;
        let curNode=root;
    ...
    inOrderTree(node:BinaryNode):void{
        let root=node;
        let curNode=root;
        let stack:Array<BinaryNode>=[curNode];
        let flag=1;
    ...
    afterOrderTree(node:BinaryNode):void{
        let root=node;
        let curNode=root;
        let stack:Array<BinaryNode>=[];
        while (true) {
    ...
    successOrNode(node:BinaryNode):BinaryNode{
        if(!node){
            return null;
        }
