...
class Chain{
    private head:ChainNode | null;
    constructor(arr:Array<any>){
        let node=arr[0]??false;
        if(node!==false){
    ...
    reverseChain(limit?:number):{head:ChainNode,next:ChainNode} | ChainNode{
        let cur=this.head;
...
function getLoop(chain:ChainNode):ChainNode | null{
    let result=null;
    if(!chain){
        return result;
    }
...
function noLoop(head1:ChainNode,head2:ChainNode):ChainNode | null{
    let n=0;
    let cur1=head1;
    let cur2=head2;
    while(cur1){
...
function bothLoop(head1:ChainNode,head2:ChainNode,loop1:ChainNode,loop2:ChainNode):ChainNode | null{
...
function getIntersectNode(head1:ChainNode | null,head2:ChainNode | null):ChainNode | null{
    if(!head1 || !head2){
        return null;
    }
...
console.log(resNode?.value);   //两个无环链表相交
...
console.log(resNode?.value);   //两个无环链表不相交
...
resNode=getIntersectNode(head1, head2);
console.log(resNode?.value);  //一个有环链表一个无环链表
...
console.log(resNode?.value);  //两个有环链表相交
...
resNode=getIntersectNode(head2,head3);
console.log(resNode?.value);  //两个有环链表不相交
